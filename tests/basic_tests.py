...
class BasicTests(unittest.TestCase):
    ...
    def basic_tests(self):
        ...
        self.db.update(docs)
        resp, data = self.db.resource.get("_fti", q="foo:document", wait=True)
    ...
    def multi_test(self):
        ...
        resp, data = self.db.resource.get("_fti", q="foo:plankton", wait=True)
        ...
        #Testing scoring
        resp, data = self.db.resource.get("_fti", q="foo:plankton bar:goat", wait=True)
        self.assertEqual(data["total_rows"], 2)
        t1score = 0.0
        t2score = 0.0
        for row in data["rows"]:
